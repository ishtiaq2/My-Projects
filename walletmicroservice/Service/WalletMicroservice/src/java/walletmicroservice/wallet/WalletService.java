...
public class WalletService {
    ...
    public Wallet getWallet(String walletId) {
        ...
        transactions.entrySet().forEach((entry) -> {
            if(entry.getValue().equals(walletId))
            transactionIds.add(entry.getKey());
        });
        Wallet wallet = wallets.get(walletId);
        wallet.setTransactionIds(transactionIds);
        return wallet;
    }
    ...
    public String debitWallet(String walletId, String transactionId, int amount) {
        try {
            ...
            if (wallet.getCurrentBalance() >= amount) {
                ...
                registerTransaction(transId, wallet, ", Debit: " + amount);
    ...
    public String creditWallet(String walletId, String transactionId, int amount) {
        try {
            ...
            wallet.setCurrentBalance(wallet.getCurrentBalance() + amount);
            String transId = getTransactionId(transactionId);
            registerTransaction(transId, wallet, ", Credit: " + amount);
    ...
    public void registerTransaction(String transId, Wallet wallet, String transaction) {
        transactions.put(transId + transaction, wallet.getId());
    }
}
